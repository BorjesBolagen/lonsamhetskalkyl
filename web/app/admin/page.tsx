...
const mockTrafficLeaders = [
  { id: 1, name: "Kalle Karlsson", email: "kalle@gmail.se", district: "Växjö", kpi: "187 kr/flm", arbetsvolym: "24 aktiva bilar" },
  { id: 2, name: "Anna Andersson", email: "anna@gmail.se", district: "Linköping", kpi: "145 kr/flm", arbetsvolym: "18 aktiva bilar" },
  { id: 3, name: "Sven Svensson", email: "sven@gmail.se", district: "Jönköping", kpi: "192 kr/flm", arbetsvolym: "32 aktiva bilar" },
];
...
export default function Admin() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: existingUser } = await supabase.from('User').select('id').eq('email', signupEmail).maybeSingle();
      if (existingUser) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#C6E2D8]">
        ...
        <div className="space-y-6 text-gray-800 font-sans w-full max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white p-6 rounded-xl shadow-md border-t-4 border-[#7ec58a]">
              <p className="text-sm text-gray-500 font-medium">Fyllnadsgrad Totalt</p>
              <p className="text-3xl font-bold mt-1">88%</p>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-md border-t-4 border-[#446E30]">
              <p className="text-sm text-gray-500 font-medium">Snitt Intäkt / FLM</p>
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border-t-4 border-gray-400">
              <p className="text-sm text-gray-500 font-medium">Körningar idag</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-md flex flex-col">
              <h3 className="font-bold mb-4 border-b-2 border-green-500 pb-2">Fyllnadsgrad per Distrikt</h3>
              <div className="flex-grow bg-gray-50 border-2 border-gray-200 rounded flex flex-col justify-end p-4 min-h-[12rem]">
                <div className="flex items-end justify-around h-full border-b-2 border-gray-300 pb-1">
                  <div className="w-12 sm:w-16 bg-[#7ec58a] h-[80%] rounded-t-sm hover:opacity-80 transition-opacity" title="Växjö: 80%"></div>
                  <div className="w-12 sm:w-16 bg-[#446E30] h-[60%] rounded-t-sm hover:opacity-80 transition-opacity" title="Linköping: 60%"></div>
                  <div className="w-12 sm:w-16 bg-[#7ec58a] h-[90%] rounded-t-sm hover:opacity-80 transition-opacity" title="Jönköping: 90%"></div>
                </div>
                <div className="flex justify-around mt-3">
                  <span className="text-xs sm:text-sm font-bold text-gray-600 w-12 sm:w-16 text-center">Växjö</span>
                  <span className="text-xs sm:text-sm font-bold text-gray-600 w-12 sm:w-16 text-center">Linköping</span>
                  <span className="text-xs sm:text-sm font-bold text-gray-600 w-12 sm:w-16 text-center">Jönköping</span>
                </div>
              </div>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-md flex flex-col">
              <h3 className="font-bold mb-4 border-b-2 border-green-500 pb-2">Intäkt per FLM</h3>
          ...
          <div className="bg-white p-6 rounded-xl shadow-md overflow-hidden">
            <h3 className="font-bold text-lg mb-4 border-b-2 border-green-500 pb-2">Trafikledare</h3>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {mockTrafficLeaders.map((user) => (
                    ...
                    >
                      <td className="p-4 text-[#446E30] font-bold">{user.name} <br/><span className="text-sm font-normal text-gray-500">{user.email}</span></td>
      ...
      {isAddUserOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md relative text-gray-800">
            <button onClick={() => setIsAddUserOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-black text-xl">✖</button>
            <h3 className="font-bold text-xl mb-6 border-b-2 border-green-500 pb-2">Skapa Ny Användare</h3>
            <form onSubmit={handleSignup} className="space-y-4">
              <div className="flex flex-col bg-gray-50 p-3 rounded-lg shadow-sm">
                <input type="email" placeholder="E-mail" value={signupEmail} onChange={(e) => setSignupEmail(e.target.value)} className="w-full p-2 border-2 border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#7ec58a]" />
              </div>
              ...
              <div className="flex flex-col bg-gray-50 p-3 rounded-lg shadow-sm">
                <select value={role} onChange={(e) => setRole(e.target.value as UserRole)} className="w-full p-2 border-2 border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#7ec58a]">
              ...
              <button type="submit" disabled={isSigningUp} className="w-full mt-4 bg-[#75C07A] text-white p-3 rounded font-bold hover:bg-green-800 transition-colors duration-300 disabled:opacity-50">
                {isSigningUp ? "Registrerar..." : "Registrera"}
              </button>
            </form>
            {signupResponse && <p className="mt-4 p-3 bg-gray-100 border-l-4 border-[#75C07A] text-sm rounded">{signupResponse}</p>}
          </div>
        </div>
      )}
      ...
      {isMessageModalOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md relative text-gray-800">
            <button onClick={() => setIsMessageModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-black text-xl">✖</button>
            <h3 className="font-bold text-xl mb-6 border-b-2 border-green-500 pb-2">Skicka info till alla</h3>
            <form onSubmit={handleSendMessage} className="space-y-4">
              <div className="flex flex-col bg-gray-50 p-3 rounded-lg shadow-sm">
                <textarea value={adminMessage} onChange={(e) => setAdminMessage(e.target.value)} placeholder="Skriv ditt meddelande här..." className="w-full p-2 border-2 border-gray-300 rounded h-32 focus:outline-none focus:ring-2 focus:ring-[#7ec58a]"></textarea>
              </div>
              <button type="submit" disabled={isSendingMessage} className="w-full bg-[#446E30] text-white p-3 rounded font-bold hover:bg-[#365926] transition-colors duration-300 disabled:opacity-50">
                {isSendingMessage ? "Skickar..." : "Skicka"}
              </button>
            </form>
            {adminResponse && <p className="mt-4 p-3 bg-gray-100 border-l-4 border-[#446E30] text-sm rounded">{adminResponse}</p>}
          </div>
        </div>
      )}
      ...
      {selectedUser && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-sm relative border-t-8 border-[#446E30] text-gray-800">
            <button onClick={() => setSelectedUser(null)} className="absolute top-4 right-4 text-gray-400 hover:text-black text-xl">✖</button>
            <h3 className="font-bold text-2xl mb-1">{selectedUser.name}</h3>
            <p className="text-gray-500 mb-6">{selectedUser.email}</p>
            <div className="space-y-3 bg-gray-50 p-4 rounded-lg">
              <p className="flex justify-between"><strong>Distrikt:</strong> <span>{selectedUser.district}</span></p>
              <p className="flex justify-between border-y border-gray-200 py-2"><strong>Prestanda:</strong> <span className="text-[#446E30] font-bold">{selectedUser.kpi}</span></p>
              <p className="flex justify-between"><strong>Arbetsvolym:</strong> <span>{selectedUser.arbetsvolym}</span></p>
            </div>
            <button onClick={() => setSelectedUser(null)} className="mt-8 w-full border-2 border-gray-300 font-bold p-3 rounded hover:bg-gray-100 transition-colors">Stäng</button>
          </div>
        </div>
      )}
