...
// GET /api/ilog/distribution-zones?date=yyyyMMdd&withEquipages=true|false
...
export async function GET(request: Request) {
  ...
  try {
    const rawZones = await ilogGet<unknown>("/ilog-api-web/distributionZones", {
      date,
      withEquipages,
    });
    ...
    if (debugRaw) {
      return NextResponse.json({
        status: true,
        message: "Raw distribution zones fetched",
        data: rawZones,
      });
    }
    ...
    return NextResponse.json({
      status: true,
      message: "Distribution zones fetched",
      data: zones,
    });
  } catch (error) {
